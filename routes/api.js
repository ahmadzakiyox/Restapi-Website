...
// Endpoint untuk mengubah API key
router.get('/changekey', async (req, res, next) => {
    var oldKey = req.query.oldkey;
    var newKey = req.query.newkey;

    if (!oldKey || !newKey) return res.json(loghandler.notparam);

    try {
        const updated = await kuhong.update({ apikey: oldKey }, { $set: { apikey: newKey, updatedAt: new Date() } });

        if (updated.n === 0) {
            return res.json({
                status: false,
                result: `Apikey "${oldKey}" tidak ditemukan!`
            });
        }

        res.json({
            status: true,
            result: `Apikey "${oldKey}" berhasil diubah menjadi "${newKey}"`
        });
    } catch (e) {
        console.log(e);
        res.sendFile(error);
...
router.get('/removekey', async (req, res, next) => {
    var apikeyInput = req.query.apikey;

    if (!apikeyInput) return res.json(loghandler.notparam);

    try {
        const removed = await kuhong.remove({ apikey: apikeyInput });

        if (removed.n === 0) {
            return res.json({
                status: false,
                result: `Apikey "${apikeyInput}" tidak ditemukan!`
            });
        }
...
router.get('/ai/turbo', async (req, res, next) => {
    var apikeyInput = req.query.apikey,
        question = req.query.question;
    ...
    try {
        // Menggunakan Hercai untuk mengajukan pertanyaan dengan model v3
        const response = await herc.question({ model: "turbo", content: question });
...
router.get('/ai/turbo-16k', async (req, res, next) => {
    var apikeyInput = req.query.apikey,
        question = req.query.question;
    ...
    try {
        // Menggunakan Hercai untuk mengajukan pertanyaan dengan model v3
        const response = await herc.question({ model: "turbo-16k", content: question });
